...
function checkARLearnForTaskChildren($group_guid) {
  ...
  debugWespotARLearn('GET TASK CHILDREN: '.print_r($gamearray, true));
  ...
  if ($gamearray === FALSE || count($gamearray) == 0) {
    ...
    if (isset($usertoken) && $usertoken != "") {
      ...
      if (isset($game->arlearn_server_time)) {
        $fromtime = $game->arlearn_server_time; //$game->last_action;
      }
      $loopcount = 0;
    ...
    if (isset($datareturned->error)) {
      ...
      if ($responsesArray && count($responsesArray) > 0) {
        ...
        foreach ($responsesArray as $response) {
          ...
          // Check if response already loaded
          $elggresponseArray = elgg_get_entities_from_metadata(array(
            'type' => 'object',
            'subtype' => 'arlearntask',
            'metadata_name' => 'arlearnid',
            'metadata_value' => $responseid,
          ));

          debugWespotARLearn('PROCESSING RESULT FOR taskArray =: '.print_r($elggresponseArray, true));

          // Don't save if we already have it
          if (!$elggresponseArray or count($elggresponseArray) == 0) {
            $responseValue = $response->responseValue;
            debugWespotARLearn("Receiving info...");
            debugWespotARLearn($response);
            ...
            if (count($users) == 1) {
              ...
              if ($taskArray && count($taskArray) > 0) {
                ...
                if ($title != "") {
                  ...
                  $result->enabled = ($response->revoked)? 'no': 'yes';
                  $result->save();
